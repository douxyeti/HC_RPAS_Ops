    ...
    def register_from_manifest(self, module_id: str, manifest_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            for screen in screens:
                ...
                if screen_id:
                    # Préparer les données de l'écran
                    screen_data = {
                        "id": screen_id,
                    ...
                    # Enregistrer l'écran
                    if self.register_screen(module_id, screen_id, screen_data):
                        success_count += 1
            
            self.logger.info(f"Module '{module_id}' et {success_count}/{len(screens)} écrans enregistrés dans Firebase")
            return True
        except Exception as e:
            self.logger.error(f"Erreur lors de l'enregistrement à partir du manifest: {str(e)}")
            return False
